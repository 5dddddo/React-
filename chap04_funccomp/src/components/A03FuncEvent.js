...
function A03FuncEvent() {
  ...
  const changeNumber = evt=> {
    ...
    // state의 변수를 참조하는 함수로도 기술 가능
    // data를 return 받고, 값 추가
    // setData(data =>{
    //   return {...data, [evt.target.name]: value}
    // })
    // 함수 한줄로 쓰기
    // setData(data =>(return {...data, [evt.target.name]: value}));
  }
    ...
    <div>
      ...
      <form>
        ...
        Age: {data.age}
          ...
          value = {data.age} onChange = {changeString}/>
        Date: {data.date}
        ...
        CheckBox One: {data.isChecked ? '동의':'비동의'}<br />
          <div className="form-check">
